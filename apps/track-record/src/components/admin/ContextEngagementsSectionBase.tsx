...
import {
  type ContextEngagementCreateInput,
  type ContextKind,
...
type ContextEngagementsSectionBaseProps = Record<string, unknown> & {
  context: EngagementSectionContext
}

function getDateDefaultsByContext(
  ...
  if (context.relationTo === 'events') {
    const eventDate = toDateInputValue(fields.eventDate?.value)
    return {
      endDate: eventDate,
      startDate: eventDate,
    }
...
export const ContextEngagementsSectionBase = ({
  context,
}: ContextEngagementsSectionBaseProps) => {
  const { id } = useDocumentInfo()
  const contextDateDefaults = useFormFields(([fields]) =>
    getDateDefaultsByContext(fields as Record<string, { value?: unknown } | undefined>, context),
  )
  const contextId =
    typeof id === 'number'
      ? id
  ...
  const resetAddParticipantForm = useCallback(() => {
    ...
    setEngagementType('')
    setEngagementStatus('')
    setStartDate(contextDateDefaults.startDate)
    setEndDate(contextDateDefaults.endDate)
    setRating('')
    setWouldRecommend('')
    setMetadataText('')
  ...
  }, [contextDateDefaults.endDate, contextDateDefaults.startDate])
  ...
  const refreshEngagements = useCallback(async () => {
    ...
    try {
      const docs = await fetchContextEngagements({
        contextId,
  ...
  const handleCreateParticipant = useCallback(
    async () => {
      ...
      try {
        ...
        const isDuplicate = await checkDuplicateContextEngagement({
          contextId,
          contextRelation: context.relationTo,
          personId,
        })
        ...
        if (isDuplicate) {
          setDuplicateError(
            `This person is already linked to this ${context.label.toLowerCase()} via an engagement.`,
          )
          return
        }

        const payload: ContextEngagementCreateInput = {
          context: {
        ...
        await createContextEngagement(payload)

        setIsAddModalOpen(false)
        setNotice(`Participant added to ${context.label.toLowerCase()}.`)
        resetAddParticipantForm()
  ...
  return (
    <section style={{ border: '1px solid var(--theme-elevation-200)', borderRadius: 8, padding: 16 }}>
      <div style={{ alignItems: 'center', display: 'flex', justifyContent: 'space-between', gap: 12 }}>
        <div>
          <h3 style={{ margin: 0 }}>{context.label} Participants (Engagements)</h3>
          <p style={{ color: 'var(--theme-elevation-500)', margin: '4px 0 0' }}>
            Manage {context.label.toLowerCase()} participants by creating and editing engagement
            records.
          </p>
        </div>
      ...
      {!canManage && (
        <div style={{ marginTop: 12 }}>
          <Banner type="info">Save {context.label.toLowerCase()} first to add participants.</Banner>
        </div>
      )}
      ...
      <div style={{ marginTop: 16 }}>
        ...
        {!isLoadingEngagements && canManage && engagementRows.length === 0 && (
          <p style={{ color: 'var(--theme-elevation-500)', margin: 0 }}>
            No {context.label.toLowerCase()} participants have been added yet.
          </p>
        )}
