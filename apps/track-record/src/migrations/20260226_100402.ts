...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   ALTER TYPE "public"."enum_engagements_type" ADD VALUE 'contribution' BEFORE 'other';`)
}
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  CREATE TYPE "public"."enum_engagements_type" AS ENUM('participant', 'facilitator', 'speaker', 'volunteer', 'organizer', 'mentor', 'other');
