...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   ALTER TYPE "public"."enum_events_type" ADD VALUE 'seminar' BEFORE 'talk';`)
}
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  CREATE TYPE "public"."enum_events_type" AS ENUM('workshop', 'talk', 'meetup', 'reading_group', 'retreat', 'panel', 'other');
