...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  FROM "programs"
  WHERE "programs"."partnership_id" = "partnerships"."id";
  DO $$
  BEGIN
    IF EXISTS (SELECT 1 FROM "partnerships" WHERE "program_id" IS NULL) THEN
      RAISE EXCEPTION 'Unable to backfill partnerships.program_id from programs.partnership_id';
    END IF;
  END $$;
